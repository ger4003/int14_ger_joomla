...
class modLbSummaryHelper {

	function getAds($catid) {
		$db = JFactory::getDBO ();
		$user = JFactory::getUser ();
		$orderby = ' ORDER BY c.modified_date desc';
		$access = ( int ) $user->get ( 'aid', 0 );
		$query = "SELECT * FROM #__gbl_ads as c WHERE category_id=$catid AND c.status='1' $orderby";
	...
	function getAdImages($adid) {
		$db = JFactory::getDBO ();
		$orderby = ' ORDER BY c.ordering asc';
		$query = "SELECT * FROM #__gbl_ads_images as c WHERE ad_id=$adid AND c.published='1' $orderby";
	...
	function getAdExtraFields($adid) {
		$db = JFactory::getDBO ();
		$query = "SELECT f.title, f.description, af.field_value FROM #__gbl_ads_fields AS af
							LEFT JOIN #__gbl_fields AS f ON af.field_id=f.id
							WHERE af.ad_id=$adid AND f.published = 1";
	...
	function getAdExtraField($fieldTitle, $adid) {
		$extraFields = modLbSummaryHelper::getAdExtraFields($adid);
	...
	function getAdPrice($adid) {
		//TODO: get price via Gobingo currency
	}
	...
	function getCategories(&$params) {
		...
		$query = "SELECT c.*,concat(c.id,':',c.alias) as slug,c.title as name,c.parent_id as parent, 0 as pcount FROM #__gbl_categories as c WHERE parent_id=0 AND c.published='1' AND c.access<=$access $orderby";
		...
		if (count ( $rows ) > 0) {
			...
			for($i = 0; $i < $n; $i ++) {
				$r = $rows [$i];
				$query2 = "SELECT *,concat(id,':',alias) as slug FROM #__gbl_categories WHERE parent_id='" . $r->id . "' AND  access<=$access";
				$db->setQuery ( $query2 );
				$r->child = $db->loadObjectList ();

				$r->adcat = self::_getChildrens ( (int)$r->id, $access );
				...
				if (count ( $r->child ) > 0) {
					...
					for($j = 0; $j < $m; $j ++) {
						$a = $r->child [$j];
						$a->adcat2 = self::_getChildrens ((int) $a->id, $access );
					}
				}
