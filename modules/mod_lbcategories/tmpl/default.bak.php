...
if(count($list)>0)
{
	...
	<a <?php echo $active;?> href="<?php echo JRoute::_("index.php?option=$option&Itemid=$listitemid&task=ads&catid=".$l->id); ?>">
	<?php 
	echo $l->title; 
	...
	if(count($l->child)>0)
	{
		foreach($l->child as $c)
		{
			...
			?>
		<dt>
		<a <?php echo $active;?> href="<?php echo JRoute::_("index.php?option=$option&Itemid=$listitemid&task=ads&catid=".$c->id); ?>">
		<?php 
		echo $c->title;  
		if($params->get('enable_count'))
		{
