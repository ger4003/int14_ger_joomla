	...
	public function onEventUserRegistered($register_id)
	{
		...
		if ($event->waiting) // registered to the waiting list
		{
			...
			if ($this->params->get('reg_mail_user', '1')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('MAIL USER REG WAITING SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('MAIL USER REG WAITING BODY', $user->name, $user->username, $event->title, $link, $SiteName);
			...
			if ($this->params->get('reg_mail_admin', '0') && $this->params->get('receivers')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('MAIL ADMIN REG WAITING SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('MAIL ADMIN REG WAITING BODY', $user->name, $user->username, $event->title, $link, $SiteName);
