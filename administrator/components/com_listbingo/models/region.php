...
class ListbingoModelRegion extends GModel {
	...
	function getList($published = false, $filter = array(), $cid = 0) {
		...
		IFNULL(ct.title,'Not Assigned') as country
	...
	function _getChildRegions($parent = 0) {
		...
		$query = "SELECT *,
		CASE WHEN CHAR_LENGTH(alias) THEN CONCAT_WS(':', id, alias) ELSE id END as slug
		from #__gbl_regions where parent_id=$parent and published='1' order by title ASC";
	...
	function getRegionsWithChild($cid = 0, $rid = 0) {
		...
		$orderby = ' ORDER BY ordering , parent_id ';
		
		$db = JFactory::getDBO ();
		$query = "SELECT *, 
		CASE WHEN CHAR_LENGTH(alias) THEN CONCAT_WS(':', id, alias) ELSE id END as slug,
		(SELECT IFNULL(id,0) FROM #__gbl_regions WHERE id=parent_id) as parentexists 
		FROM #__gbl_regions $condition $orderby";
		$db->setQuery ( $query );
		...
		if (count ( $regions ) > 0) {
			...
			foreach ( $regions as $r ) {
				...
				/*	if ($r->parent_id > 0 && $r->parentexists) {
					$regions [$r->parent_id]->child [] = $r;
					$regions [$r->parent_id]->childrencount ++;
					array_push ( $removearray, $regions [$r->id] );
				} elseif ($r->parent_id > 0 && ! $r->parentexists) {
					array_push ( $removearray, $regions [$r->id] );
			...
			foreach ( $removearray as &$remarr ) {
				if ($rid != $remarr->id) {
					unset ( $regions [$remarr->id] );
				}
			}
		}
		
		if ($rid > 0) {
			if (isset ( $regions [$rid]->child )) {
				
				return $regions [$rid]->child;
			} else {
				return array ();
			}
		} else {
	...
	function getRegionTitle(&$params = null) {
		...
		$countrystate = ( int ) $mainframe->getUserState ( $option . 'country' );
		
		$regionstate = ( int ) $mainframe->getUserState ( $option . 'region' );
		...
		$query = "SELECT *,
		CASE WHEN CHAR_LENGTH(alias) THEN CONCAT_WS(':', id, alias) ELSE id END as slug
		FROM #__gbl_regions WHERE id=" . ( int ) $regionstate . " $cond";
	...
	function getSlug($id = 0) {
		...
		CASE WHEN CHAR_LENGTH(alias) THEN CONCAT_WS(':', id, alias) ELSE id END as slug 
		FROM #__gbl_regions WHERE id=$id";
		$db->setQuery ( $query );
		return $db->loadObject ();
	}
	...
	function getId($name = '') {
		...
		$query = "SELECT id,
		CASE WHEN CHAR_LENGTH(alias) THEN CONCAT_WS(':', id, alias) ELSE id END as slug 
		FROM #__gbl_regions WHERE title='$name'";
		$db->setQuery ( $query );
		return $db->loadObject ();
	}
