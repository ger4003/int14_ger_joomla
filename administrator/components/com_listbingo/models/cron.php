...
class ListbingoModelCron extends GModel {
	...
	function expire($params = null) {
		...
		$query = "SELECT  * from #__gbl_ads where expiry_date< now()  and expiry_date!='$nulldate' and status=1 and id>" . ( int ) $tracker->lastrunid . " order by id limit $count";
		$db->setQuery ( $query );
	...
	function alertExpire($params = null) {
		...
		$db = JFactory::getDBO ();
		$nulldate = $db->getNullDate ();
		$query = "SELECT  * from #__gbl_ads where DATE_ADD(expiry_date, INTERVAL -$alertbefore DAY)< now()  and expiry_date!='$nulldate' and status=1 and id>" . ( int ) $tracker->lastrunid . " order by id limit $count";
		$db->setQuery ( $query );
		$rows = $db->loadObjectList ();
		$lastrun = 0;
	...
	function archive($params = null) {
		...
		if ($autoarchive) {
			...
			$query = "SELECT  * from #__gbl_ads where DATE_ADD(expiry_date, INTERVAL $archivedays DAY)< now()  and expiry_date!='$nulldate' and id>" . ( int ) $tracker->lastrunid . " order by id limit $count";
			$db->setQuery ( $query );
			$rows = $db->loadObjectList ();
			...
			if (count ( $rows ) > 0) {
				foreach ( $rows as $r ) {
					...
					GApplication::triggerEvent ( 'onAdArchive', array (&$ad, & $params ) );
					$lastrun = $r->id;
				}
			}
