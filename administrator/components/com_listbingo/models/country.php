...
class ListbingoModelCountry extends GModel {
	...
	function getCountryLists($published = false, $filter = array()) {
		...
		$db = JFactory::getDBO ();
		$query = "SELECT *,
		CASE WHEN CHAR_LENGTH(code) THEN CONCAT_WS(':', id, code) ELSE id END as slug
		FROM #__gbl_countries $pubcond $orderby";
	...
	function getList($published = false, $filter = array()) {
		...
		$query = "SELECT *,
		CASE WHEN CHAR_LENGTH(code) THEN CONCAT_WS(':', id, code) ELSE id END as slug
		from #__gbl_countries $where $orderby";
	...
	function getCountryTitle(&$params = null) {
		...
		CASE WHEN CHAR_LENGTH(code) THEN CONCAT_WS(':', id, code) ELSE id END as slug
		FROM #__gbl_countries WHERE id=".(int)$countrystate;
	...
	function findAndSetCountry(&$title = null, $setregion = true) {
		...
		CASE WHEN CHAR_LENGTH(code) THEN CONCAT_WS(':', id, code) ELSE id END as slug
		from #__gbl_countries $condition";
		$db->setQuery ( $query );
