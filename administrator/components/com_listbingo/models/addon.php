...
class ListbingoModelAddon extends GModel {
	...
	function getList($published = false, $filter = array()) {
		...
		$db = JFactory::getDBO ();
		$query = "SELECT * from #__gbl_addons $condition $orderby";
		...
		$adminDir = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_listbingo' . DS . 'addons';
		
		$numRows = count ( $rows );
		for($i = 0; $i < $numRows; $i ++) {
			$row = & $rows [$i];
			
			/* Get the addon folder and list of xml files in folder */
			$folder = $adminDir . DS . $row->folder;
			...
			if (count ( $xmlFilesInDir )) {
				$data = JApplicationHelper::parseXMLInstallFile ( $folder . DS . $row->element . '.xml' );
				if ($data) {
