...
class ListbingoModelProfile extends GModel {
	...
	function loadProfile($userid = null, $params = null) {
		
		$db = JFactory::getDBO ();
		
		$displayname=$params->get('lb_display_name','name');
				
		$query = "SELECT p.*, u.$displayname as name, u.email, u.registerDate, r.title as rtitle, c.title as ctitle,
	...
	function save($post = null, $file = null, $params = null) {
		...
		$db = JFactory::getDBO ();
		$query = "UPDATE #__users set name='" . $post ['name'] . "', email='" . $post ['email'] . "' WHERE id='" . $post ['user_id'] . "'";
		$db->setQuery ( $query );
