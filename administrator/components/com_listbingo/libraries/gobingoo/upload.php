...
class upload {
	...
	function process($server_path = null) {
		$red = $green = $blue = "FF";
		$red1 = $green1 = $blue1 = "FF";
		$red2 = $green2 = $blue2 = "FF";
		$tint_red = $tint_green = $tint_blue = 'FF';
		$this->error = '';
		$this->processed = true;
		$return_mode = false;
		$return_content = null;
...
class GUpload {
	...
	function __construct() {
		...
		global $option;
		$xoption = trim ( str_replace ( "com_", "", $option ) );
		$this->uploadpath = JPATH_ROOT . DS . "images" . DS . $xoption;
		$this->uploadpath = str_replace ( "/", DS, $this->uploadpath );
		$size = ini_get ( 'upload_max_filesize' );
		$this->maxfilesize = $size * 1024 * 1024;
	
	}
	
	function setMaxFileSize($size = null) {
		if (is_null ( $size )) {
			return false;
		}
	...
	function upload($file = null, &$return = null) {
		if (is_null ( $file )) {
			return false;
		}
	}
	...
	function makefilename($filename) {
		$prefix = str_replace ( " ", "_", $filename );
		$prefix = str_replace ( "-", "_", $prefix );
		return $prefix;
	
	}
	
	function setFolderPermission($params = null) {
		if ($params->get ( 'usesystemfolderpermissions' )) {
	...
	function uploadArray($files = array(), $params = null) {
		...
		if (isset ( $params->prefix )) {
			$prefix = self::makefilename ( $params->prefix );
		}
		
		if (isset ( $params->uploadfolder ) && ! empty ( $params->uploadfolder )) {
			$this->uploadpath = $params->uploadfolder;
			$this->uploadpath = str_replace ( "/", DS, $this->uploadpath );
		}
		...
		for($i = 0; $i < $count; $i ++) {
			...
			$name = each ( $files ['name'] );
			$type = each ( $files ['type'] );
			$tmp_name = each ( $files ['tmp_name'] );
			$error = each ( $files ['error'] );
			$size = each ( $files ['size'] );
			
			$filesforupload [$i] ['name'] = $name ['value'];
			$filesforupload [$i] ['fid'] = $name ['key'];
			$filesforupload [$i] ['type'] = $type ['value'];
			$filesforupload [$i] ['tmp_name'] = $tmp_name ['value'];
			$filesforupload [$i] ['error'] = $error ['value'];
			$filesforupload [$i] ['size'] = $size ['value'];
		
		}
		
		if (count ( $filesforupload ) > 0) {
			
			foreach ( $filesforupload as $f ) {
				
				$filename = $prefix . strtolower ( $f ['name'] );
				$filename = str_replace ( " ", "_", $filename );
				
				//	$filename=$prefix.rand(0,99999);
				

				$uploader = new upload ( $f );
				$uploader->file_max_size = $this->maxfilesize;
				//$uploader->dir_auto_chmod=true;
				$uploader->dir_chmod = 0777;
				...
				if ($uploader->uploaded) {
					...
					$uploader->Process ( $this->uploadpath );
					$eachfile = new stdClass ();
					$eachfile->filename = $uploader->file_dst_name;
					
					$eachfile->fid = $f ['fid'];
					$eachfile->extension = $uploader->file_dst_name_ext;
					$eachfile->filesize = $uploader->file_src_size;
					$eachfile->mimetype = $uploader->file_src_mime;
					
					if (! $uploader->processed) {
						
						$eachfile->uploaded = false;
						$failed ++;
					
					} else {
						
						$eachfile->uploaded = true;
						$success ++;
	...
	function thumbnail($file = null, $params = null) {
		/**
		 *
		 */
		...
		if (isset ( $params->saveoriginal )) {
			$saveoriginal = $params->saveoriginal;
		}
		...
		if ($uploader->uploaded) {
			...
			$eachfile = new stdClass ();
			$eachfile->filename = $filename;
			$eachfile->extension = $uploader->file_dst_name_ext;
			$eachfile->filesize = $uploader->file_src_size;
			$eachfile->mimetype = $uploader->file_src_mime;
			$convert = isset ( $params->convert ) ? $params->convert : 'jpg';
			$eachfile->converted_to = $convert;
			$uploader->image_convert = $convert;
			
			if ($uploader->file_is_image) {
				$uploader->file_new_name_body = $filename;
				
				if ($saveoriginal || (count ( $params->thumbnails ) <= 0)) {
					
					$uploader->image_convert = $convert;
					$uploader->Process ( $this->uploadpath );
					
					$return->original [] = $eachfile;
				}
				if (count ( $params->thumbnails ) > 0) {
					$sizecount = 1;
					foreach ( $params->thumbnails as $thumbnail ) {
						$suffix = isset ( $thumbnail->suffix ) ? $thumbnail->suffix : "_" . $sizecount;
						$resize = isset ( $thumbnail->resize ) ? $thumbnail->resize : true;
						$ratio = isset ( $thumbnail->ratio ) ? $thumbnail->ratio : false;
						$imagex = isset ( $thumbnail->x ) ? $thumbnail->x : false;
						$imagey = isset ( $thumbnail->y ) ? $thumbnail->y : false;
						
						$width = isset ( $thumbnail->width ) ? $thumbnail->width : '200';
						$height = isset ( $thumbnail->height ) ? $thumbnail->height : '150';
						$crop = isset ( $thumbnail->crop ) ? $thumbnail->crop : false;
						$cropfrom = isset ( $thumbnail->cropfrom ) ? $thumbnail->cropfrom : false;
						
						$uploader->file_new_name_body = $filename;
						$uploader->file_name_body_add = $suffix;
						$uploader->image_resize = true;
						$uploader->image_ratio = $ratio;
						$uploader->image_x = $width;
						$uploader->image_y = $height;
						
							if ($crop) {
										
										$uploader->image_ratio_crop = $cropfrom;
									} else {
										
										$uploader->image_ratio_y = $imagex;
										$uploader->image_ratio_x = $imagey;
									}
	...
	function thumbnailArray($files = array(), $params = null) {
		/**
		 *
		 */
		if (isset ( $files ['name'] )) {
			...
			for($i = 0; $i < $count; $i ++) {
				...
				$curimg = array ();
				$curimg ['name'] = $files ['name'] [$i];
				$curimg ['type'] = $files ['type'] [$i];
				$curimg ['tmp_name'] = $files ['tmp_name'] [$i];
				$curimg ['error'] = $files ['error'] [$i];
				$curimg ['size'] = $files ['size'] [$i];
				$filesforupload [] = $curimg;
			}
			...
			if (count ( $filesforupload ) > 0) {
				...
				foreach ( $filesforupload as $f ) {
					...
					if ($uploader->uploaded) {
						...
						$eachfile->filesize = $uploader->file_src_size;
						$eachfile->mimetype = $uploader->file_src_mime;
						$eachfile->converted_to = $params->convert;
