...
class JTableProfile extends GTable {
	...
	function check($post) {
		...
		if (empty ( $this->country_id )) {
			$error [] = JText::_ ( 'COUNTRY_REQUIRED' );
		
		}
		
		if (empty ( $this->region_id )) {
			$error [] = JText::_ ( 'REGION_REQUIRED' );
	...
	function saveImage($image = null, $params) {
		...
		$thumbnail->cropfrom = $params->get ( 'profile_cropfrom_sml' );
		...
		GApplication::message ( $returns->success . " " . JText::_ ( "IMG_UPLOAD_SUCCESS" ), 'success' );
		
		return true;
	
	}
	
	function removeImage($userid, $params = null) {
		global $option;
		$model = gbimport ( 'listbingo.model.profile' );
		$profile = $model->loadProfile ( $userid, $params );
		$db = JFactory::getDBO ();
