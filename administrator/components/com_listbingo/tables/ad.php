...
class JTableAd extends GTable {
	 ...
	 */
	function check($params = null, $image = array()) {
		...
		if (empty ( $this->title )) {
			$error [] = JText::_ ( 'AD_MUST_HAVE_TITLE' );
		
		}
		
		if ($params->get ( 'enable_field_address1' ) && $params->get ( 'required_field_address1' ) && empty ( $this->address1 )) {
			$error [] = JText::_ ( 'ADDRESS1_REQUIRED' );
		}
		
		if ($params->get ( 'enable_field_address2' ) && $params->get ( 'required_field_address2' ) && empty ( $this->address2 )) {
			$error [] = JText::_ ( 'ADDRESS2_REQUIRED' );
		}
		
		if ($params->get ( 'enable_field_metadesc' ) && $params->get ( 'required_field_metadesc' ) && empty ( $this->metadesc )) {
			$error [] = JText::_ ( 'META_REQUIRED' );
		}
		
		if ($params->get ( 'enable_field_tags' ) && $params->get ( 'required_field_tags' ) && empty ( $this->tags )) {
			$error [] = JText::_ ( 'TAG_REQUIRED' );
		}
		
		$imgflag = 0;
		foreach ( $image ['name'] as $key => $imgs ) {
			if (! empty ( $imgs )) {
				 $imgflag = $imgflag + 1;
			}
		}
		
		if ($params->get ( 'enableimages' ) && $params->get ( 'required_field_images' ) && $imgflag == 0 && ! $this->id) {
			$error [] = JText::_ ( 'IMAGE_REQUIRED' );
		}
		
		if (empty ( $this->metadesc )) {
			$this->metadesc = GHelper::truncate ( strip_tags($this->description), 255, '' );
		}
		
		if ($params->get ( 'enable_field_price',0)) {
						
			if (isset ( $params->hasprice ) && $params->hasprice) {
				
				if (empty ( $this->pricetype )) {
					$error [] = JText::_ ( 'PRICE_TYPE' );
				
				} else {
					if ($this->pricetype == 1) {
						
						if (empty ( $this->price ) || empty ( $this->currency ) || $this->price<=0) {
							$error [] = JText::_ ( 'PRICE_REQUIRED' );
						}
					}
				}
			}
		...
		$this->tags = $filter->clean ( strip_tags ( $this->tags ), "STRING" );
		$this->metadesc = $filter->clean ( strip_tags ( $this->metadesc ), "STRING" );
	...
	function loadWithFields($id = 0, $infobar = false) {
		...
		if($params->get('show_expired_ad',0)==0 && $this->expiry_date!=$db->getNullDate ())		
		{
			
			$date1 = date ( 'Y-m-d' );
			$date2 = date ( 'Y-m-d', strtotime ( $this->expiry_date ) );
		...
		if (($this->status == 1 && $expired == 0) || $this->user_id == $userid || $app->isAdmin ()) {
			...
			$db->setQuery ( $query );
			$efrows = $db->loadObjectList ();
			if (count ( $efrows )) {
				foreach ( $efrows as &$ef ) {
					$ef->value = JFilterOutput::cleanText ( $ef->value );
				}
			}
			
			$this->extrafields = $efrows;
