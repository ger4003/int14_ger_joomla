...
class ListbingoViewAds extends GView {
	function display($tpl = null) {
		...
		$filter->status = $mainframe->getUserStateFromRequest ( $option . 'adsfilter_status', 'status', -1, 'int' );

		
		$filter->featured = JRequest::getInt ( 'featured', 0 );
		$filter->category_id = $mainframe->getUserStateFromRequest ( $option . 'adsfilter_category_id', 'category_id', 0, 'int' );

		$filter->country = $mainframe->getUserStateFromRequest ( $option . 'adsfilter_country_id', 'country_id', 0, 'int' );
		...
		$lists ['new'] = JHTML::_ ( 'select.booleanlist', 'new', array ('class' => "inputbox", 'onclick' => "document.adminForm.submit();" ), JRequest::getInt ( 'new', 0 ) );
