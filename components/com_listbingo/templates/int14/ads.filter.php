...
$changelink=JRoute::_("index.php?option=$option&task=regions.select&type=country&Itemid=$listitemid");


$latestlink=JRoute::_("index.php?".ListbingoHelper::getSearchUrl("latest",$this->filter->orderingtype));
$pricelink=JRoute::_("index.php?".ListbingoHelper::getSearchUrl("price",$this->filter->orderingtype));
$statelink=JRoute::_("index.php?".ListbingoHelper::getSearchUrl("state",$this->filter->orderingtype));
...
{
	$searchtxt = $this->filter->searchtxt;
	if($searchtxt=="")
	{
		$searchtxt = JText::_('ALL');
	}
	...
	echo JText::sprintf("RESULTS_FOR",$searchtxt.$cat,$this->filter->regiontitle);
}
else
{
