...
var maxlimit_m = parseInt(<?php
	echo $this->maxlimit;
	?>);
var alertmsg_m = '<?php echo JText::_ ( 'IMAGE_UPLOAD_EXCEED' ); ?>';

var count_alert = '<?php echo JText::_ ( 'SELECT_ATLEAST_ONE_IMAGE' ); ?>';
...
<form name="adminForm" id="adminForm" action="<?php echo JRoute::_ ( 'index.php?option='.$option.'&task=ads.deleteImages&Itemid='.$listitemid ); ?>" method="post">
...
<input type="button" class="gbButton validate" name="delete" id="deleteBtn" value="<?php echo JText::_('DELETE');?>" /> 
	...
	onclick="checkAll(<?php echo count ( $this->row->images ); ?>);" /><?php
...
<input type="hidden" name="task" value="ads.deleteImages" />
</form>
</div>
...
<fieldset><legend><strong><?php
	...
	?></strong></legend>
<form action="<?php echo JRoute::_ ( 'index.php?option='.$option.'&task=ads.uploadImagesItemid='.$listitemid ); ?>" method="post"
...
<ul>
	...
	<div class="remove" id="gb_manage_remove_image" style="display: none;"><a><?php echo JText::_('REMOVE');?></a></div>
	</li>
...
<?php
	...
	echo JText::_ ( 'IMAGE_UPLOAD_EXCEED' );
}
