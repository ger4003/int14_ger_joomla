...
class ListbingoViewProfile extends GTemplate {
	function display($tpl = null) {
		...
		$countries1 = array ();
		$countries1 [] = JHTML::_ ( 'select.option', '', JText::_ ( 'SELECT_COUNTRY' ), 'value', 'text' );
		...
		if (! file_exists ( $basepath . $row->image . $params->get ( 'suffix_profile_image' ) . "." . $row->extension )) {
			...
			if ($row->image != "") {
				$dellink = JRoute::_ ( 'index.php?option=' . $option . '&Itemid=' . $listitemid . '&task=profile.removeImage&time=' . time () );
