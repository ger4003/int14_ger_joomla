		...
		if (! $row->canbeviewed || is_null ( $row->user_id )) {
			...
			if (! empty ( $row->address->address )) {
				if (! empty ( $row->address->zipcode )) {
					 $address [] = $row->address->zipcode . ", " . $row->address->address;
				}
				else {
			...
			if (($row->user_id == $user->get ( 'id', 0 )) && ! $user->guest) {
				$link = JRoute::_ ( "index.php?option=$option&Itemid=$listitemid&task=ads.edit&catid=" . $row->category_id . "&adid=" . $row->id, false );
				$row->adlink = "<a rel=\"nofollow\" href=\"$link\">" . JText::_ ( 'EDIT' ) . "</a>";
			} else {
				$link = JRoute::_ ( "index.php?option=$option&&task=new&Itemid=$listitemid", false );
				$row->adlink = "<a rel=\"nofollow\" href=\"$link\">" . JText::_ ( 'POST' ) . "</a>";
			}
			...
			$row->tags = JFilterOutput::cleanText ( $row->tags );
			...
			if (is_object ( $row->aduser )) {
				$row->aduser->profilelink = "<a href=" . $row->aduser->profilelink . ">" . ucfirst ( $row->aduser->$displayname ) . "</a>";
			...
			$row->metadesc = JFilterOutput::cleanText ( $row->metadesc );
			
			//$row->extrafields = JFilterOutput::objectHTMLSafe ( $row->extrafields );
			//$row->images = JFilterOutput::objectHTMLSafe ( $row->images );
			...
			$expired = 0;
			$db = JFactory::getDBO();
			if($row->expiry_date!=$db->getNullDate())			
			{
				$date1 = date ( 'Y-m-d' );
				$date2 = date ( 'Y-m-d', strtotime ( $row->expiry_date ) );
